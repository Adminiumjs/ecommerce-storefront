...
`docker compose up` gives you a working product: `shop-db` creates the
...
By default these talk to the `shop-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/shop npm run demo:status
```
